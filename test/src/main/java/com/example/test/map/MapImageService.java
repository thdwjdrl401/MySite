...
public class MapImageService {
	...
	private final MapDetailImageRepository mapDetailImageRepository;
...
//			 이미지가 없으면 객체 생성 후 저장
	...
	public MapDetailImagesResponseDTO getDetailImageDTO(String location) throws Exception {

		String defaultImageUrl = "/mapImage/black.png";
		GetJson json = new GetJson();
		List<String> locations = json.readJson(location);
		List<String> locations_kr = json.readJson_kr(location);
		log.info("locations_kr = " + locations_kr);
		List<MapDetailImage> imageList = new ArrayList<MapDetailImage>();	
		log.info("locations.size() = " + locations.size());

		for (int i = 1; i <= locations.size(); i++) {

			try {
				MapDetailImage image = mapDetailImageRepository.findByLocation(locations.get(i - 1));

				if (image.getUrl() == null) {

					image.setLocation(locations.get(i - 1));

					image.setLocation_kr(locations_kr.get(i - 1));

					image.setUrl(defaultImageUrl);

					mapDetailImageRepository.save(image);
				...
				MapDetailImage image = new MapDetailImage();
				...
				this.mapDetailImageRepository.save(image);

			}

		}

		return MapDetailImagesResponseDTO.builder().mapDetailImage(imageList).build();
	}
	
	
	
	public void uploadDetail(MapImageUploadDTO imageUploadDTO, String location) {
		MultipartFile file = imageUploadDTO.getFile();
		...
		try {
			file.transferTo(destinationFile);
			MapDetailImage mapImage = mapDetailImageRepository.findByLocation(location);
			if (mapImage.getUrl() == "/mapImage/black.png") {
				// 이미지가 이미 존재하면 url 업데이트
				File deletefile = new File("C:/Temp" + mapDetailImageRepository.findByLocation(location).getUrl());
				if (deletefile.exists() == false)
					deletefile = new File("home/lazy/Temp" + mapDetailImageRepository.findByLocation(location).getUrl());
				deletefile.delete();
				mapImage = mapDetailImageRepository.findByLocation(location);
				mapImage.updateUrl("/mapImage/" + imageFileName);
...
//			 이미지가 없으면 객체 생성 후 저장
			...
			mapDetailImageRepository.save(mapImage);
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}
	...
	public MapDetailImageResponseDTO getDetailIDetailmageDTO(String location) {

		MapDetailImage image = mapDetailImageRepository.findByLocation(location);

		return MapDetailImageResponseDTO.builder().mapDetailImage(image).build();
